...
window.onload = function() {
				...
				btn.onclick = function() {
					...
					//一般我们读取元素的样式，不能通过这样的方法去读，因为起始元素的样式不是写在行内的，获取不到
					// console.log(box.style.width);//一开始读不到，因为这样是从行内去读的，而起始行内并没写样式，读取一个空串
				}
			}
```
...
window.onload = function() {
				...
				var flag = true; //true就代表当前风车在转，false就代表风车暂停着
				btn.onclick = function() {
					if (flag) {
...
<html>
	...
	<body>
		...
		<script type="text/javascript">
			window.onload = function() {
				//var pNodes = document.querySelectorAll('p');
				//这个获取元素的方法及getElementsByTagName、getElementsByClassName都是低级浏览器不支持
...
<html>
	...
	<body>
		...
		<script type="text/javascript">
			//如果用户使用的是高级浏览器就自动的通过textContent去操作内容
			//如果用户使用的是低级浏览器就自动的通过innerText去操作内容
			var pNode = document.getElementById('pp');
			//			console.log(pNode.textContent);不能直接写，因为这样只能满足高级浏览器的用户操作内容，低级浏览器不能
			//			console.log(getOrSetContent(pNode));//兼容性获取P内部的内容
			getOrSetContent(pNode, '老马过河'); //兼容性设置p内部的内容
		...
		<style>
			*{
			...
			#box .back{
				...
				height: 300px;
				background-color: blue;
				/*如果子元素做了3d旋转而父元素没有开启3d空间
				 那么3d旋转后的子元素比没有3d旋转的定位（但没有z-index）子元素，层级要高一点
				 如果开启了父元素的3d空间，那么层级就正常了；*/
