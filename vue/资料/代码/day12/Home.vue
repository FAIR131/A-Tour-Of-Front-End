  ...
  <el-container>
    <el-aside width="200px">
      ...
      <el-menu
        ...
        @close="handleClose"
      >
        <el-submenu :index="'/home'+item.path" v-for="item in menus" :key="item.path">
          <template slot="title">
            <i :class="item.icon"></i>
            <span>{{item.name}}</span>
          </template>

          <el-menu-item :index="subitem.path" v-for="subitem in item.children" :key="subitem.path"> <i :class="subitem.icon"></i> {{subitem.name}}</el-menu-item>
