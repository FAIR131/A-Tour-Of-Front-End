...
import { Table, Button, Modal, Descriptions, Tag, Tree, message, Form, Input, Select } from 'antd';
...
function RoleManage(props) {
    ...
    let initData = (pageNum = 1, pageSize = 10) => {
        http.get("/sysUser/getAllRoles", { params: { pageNum, pageSize } }).then(({ data: { object } }) => {
    ...
    return (
        <>
            ...
            <Modal title="角色授权" width={"80%"} open={grantModalOpen} onOk={() => { setGrantModalOpen(false) }} onCancel={() => { setGrantModalOpen(false) }}>
